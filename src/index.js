...
_.each = function<T>(list:Array<T>, callback:(T,number,Array<T>)=> any): void {
  if (isArray(list)) {
...
_.map = function <T>(list: Array<T>, callback:(T,number,Array<T>)=>T) :Array<T>{
  const result = [];
  _.each(list, (v, i, l) => {
...
_.filter = function<T> (list:Array<T>, callback:(T,number,Array<T>)=>T):Array<T> {
  const result = [];
  _.each(list, (v, i, l) => {
    if (callback(v, i, l)) {
      _.push(result, v);
...
_.reduce = function <T>(list:Array<T>, callback:(T,T)=>T, initial:T):T {
...
_.findIndex = function <T>(list :Array<T>, callback:(T)=>T) : ?number {
  let index;
  _.each(list, (v, i, l) => {
    if (callback(v)) {
      index = i;
